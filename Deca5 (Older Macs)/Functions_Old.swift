...
func jailbreak_main() -> Int {
    ...
    dispatchQueue.async(group: group,  execute: {
        ...
        DispatchQueue.global(priority: .default).async {
            sendModel.callback?("Downloading Jailbreak Ramdisk...")
            download_ramdisk(as_path: ecid_path, version: major ?? "", completion: {result in
                if result {
                    g.leave()
                } else {
                    sendModel.callback?("Error downloading Ramdisk")
                    sendModel.try_again_show?(true)
                    g.leave()
                    return
                }
        ...
        sendModel.callback?("Patching Kernel...")
        ...
        if ret != 0 {
            sendModel.callback?("Error patching kernel")
            sendModel.try_again = true
            sendModel.try_again_show?(true)
            sendModel.progress?(0.0)
            return
        ...
        jailbreak_613(as_path: ecid_path, device: "\(device)")
        ...
        sendModel.progress?(0.0)
        sendModel.callback?("Preparing to install Jailbreak...")
        ...
        sendModel.callback?("Successfully booted device...installing Jailbreak")
        sendModel.progress?(100)
...
func download_ramdisk(as_path: URL, version: String, completion: @escaping (Bool) -> Void) {
...
func jailbreak_613(as_path: URL, device: String) {
    ...
    for component in componenets {
        ...
        var path = ""
        var ret: Int32
        sendModel.callback?("Entering \(component)...")
        group.enter()
        DispatchQueue.global(priority: .default).async {
        ...
        if component == "DeviceTree" {
            sendModel.callback?("Downloading \(component)...")
        ...
        decrypt(convert_to_mutable_pointer(value: "\(as_path.path)/\(component)6.download"), convert_to_mutable_pointer(value: "\(as_path.path)/\(component)6.decrypted"), convert_to_mutable_pointer(value: key), convert_to_mutable_pointer(value: iv), convert_to_mutable_pointer(value: "FALSE"), UnsafeMutablePointer<Int8>(mutating: nil))
            if component == "Kernelcache" {
                sendModel.callback?("Patching \(component)...")
                ...
                if ret != 0 {
                    sendModel.callback?("Error patching \(component)...")
